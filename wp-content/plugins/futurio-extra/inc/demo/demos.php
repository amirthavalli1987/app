...
if (!class_exists('FuturioWP_Demos')) {
    ...
    class FuturioWP_Demos {
        ...
        public function init() {
			...
			// Import widgets
            add_action('wp_ajax_futurio_ajax_activate_theme', array($this, 'ajax_activate_theme'));
        ...
        public static function get_demos_data() {
            ...
            $data = array(
				'futurio-storefront' => array(
                    'categories' => array('WooCommerce', 'Elementor', 'Business', 'Free'),
                    'xml_file' => $url . 'futurio-storefront/sample-data.xml',
                    'theme_settings' => $url . 'futurio-storefront/futurio-export.json',
                    'widgets_file' => $url . 'futurio-storefront/widgets.wie',
                    'home_title' => 'Home Futurio Storefront',
                    'blog_title' => 'Blog',
                    'posts_to_show' => '6',
					...
					'required_theme' => 'futurio-storefront',
					'required_theme_url' => 'https://futuriowp.com/futurio-storefront/',
                ),
				'woocommerce-tech' => array(
                    'categories' => array('WooCommerce', 'Elementor', 'Business', 'Free'),
                    'xml_file' => $url . 'woocommerce-tech/sample-data.xml',
                    'theme_settings' => $url . 'woocommerce-tech/futurio-export.json',
                    'widgets_file' => $url . 'woocommerce-tech/widgets.wie',
                    'home_title' => 'Home Tech',
                    'blog_title' => 'Blog',
                    'posts_to_show' => '6',
                    'elementor_width' => '1140',
					'is_shop' => true,
        ...
        public static function ajax_demo_data() {
			...
			// Get required theme
			$note = isset($demos[$demo]['note']) ? $demos[$demo]['note'] : '' ;
                ...
                <div class="fwp-popup-text">
                    ...
                    <div class="fwp-required-plugins-wrap">
						...
						<?php if ((isset($required_theme) && !empty($required_theme)) && ($theme->template != $required_theme) ) { ?>
							...
							<p><?php echo sprintf( esc_html__('This demo requires a different active theme. Theme %1$s will be activated after the import.', 'futurio-extra'), '<a href="' . esc_url($required_theme_url) . '" target="_blank">' . esc_html(ucwords(str_replace('-',' ',$required_theme))) . '</a>');?></p>
                        <?php } ?>
						<?php if ((isset($note) && !empty($note)) ) { ?>
							<p><?php wp_kses_post($note);?></p>
                        <?php } ?>		
                    </div>
            ...
            <form method="post" id="fwp-demo-import-form">
                ...
                <div class="fwp-demo-import-form-types">
                    ...
                    <ul class="fwp-popup-text">
						...
						<?php if ((isset($required_theme) && !empty($required_theme)) && ($theme->template != $required_theme) ) { ?>
							<li>
								<label for="futurio_activate_theme">
									<input id="futurio_activate_theme" type="checkbox" name="futurio_activate_theme" checked="checked" disabled/>
									<strong><?php esc_html_e('Activate Required Theme', 'futurio-extra'); ?></strong>
								</label>
							</li>
						<?php }	?>
                    </ul>
		...
		private function tbfe_plugin_fire() {
            ...
            $demo_type = isset($_POST['futurio_import_demo']) ? sanitize_text_field( wp_unslash( $_POST['futurio_import_demo'] ) ) : '';
			...
			$plugins = array(
				array('name' => $required_theme, 'path' => esc_url('http://futuriowp.com/wp-content/uploads/download/'.$required_theme.'.zip')),
			);
			$this->tbfe_get_plugins($plugins);
		}

		public function tbfe_plugin_unpack($args, $target, $name) {
		...
		public function tbfe_plugin_download($url, $path) {
			$content = file_get_contents($url);
			file_put_contents($path, $content);
		}

		public function tbfe_plugin_activate($installer) {
			 // Get the selected demo
            $demo_type = isset($_POST['futurio_import_demo']) ? sanitize_text_field( wp_unslash( $_POST['futurio_import_demo'] ) ) : '';
			$result = '';
            // Get demos data
			...
			$current_theme_folder = is_dir( WP_CONTENT_DIR . '/themes/'.$required_theme.'/' );
		...
		public function tbfe_get_plugins($plugins) {
			...
			foreach ($plugins as $plugin) {
				$this->tbfe_plugin_download($plugin['path'], $args['path'].$plugin['name'].'.zip');
				$this->tbfe_plugin_unpack($args, $args['path'].$plugin['name'].'.zip', $plugin['name']);
				$this->tbfe_plugin_activate($plugin['name']);
			}
		}
        ...
        public function ajax_get_import_data() {
            ...
            echo json_encode(
                    array(
						array(
                            'input_name' => 'futurio_activate_theme',
                            'action' => 'futurio_ajax_activate_theme',
                            'method' => 'ajax_activate_theme',
                            'loader' => esc_html__('Activating Required Theme', 'futurio-extra')
                        ),
                        array(
        ...
        public function ajax_reset_mods() {
            if (!isset( $_POST['futurio_import_demo_data_nonce'] ) || !wp_verify_nonce($_POST['futurio_import_demo_data_nonce'], 'futurio_import_demo_data_nonce')) {
                die('This action was stopped for security purposes.');
            }
            $save_result = 1;
            // Get the selected demo
            $demo_type = isset($_POST['futurio_reset_mods']) ? sanitize_text_field( wp_unslash( $_POST['futurio_reset_mods'] ) ) : '';
