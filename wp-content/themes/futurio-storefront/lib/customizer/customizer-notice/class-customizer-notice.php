<?php
/**
 * Futurio Storefront Customizer Notification System.
 *
 * @package Futurio
 */

/**
 * Futurio Storefront Customizer Notify Class
 */
class Futurio_Storefront_Customizer_Notice {
	...
	/**
	 * The single instance of Futurio_Storefront_Customizer_Notice
	 *
	 * @var Futurio_Storefront_Customizer_Notice $instance The Futurio_Storefront_Customizer_Notice instance.
	 */
	...
	/**
	 * The Main Futurio_Storefront_Customizer_Notice instance.
	 *
	 * We make sure that only one instance of Futurio_Storefront_Customizer_Notice exists in the memory at one time.
	 *
	 * @param array $config The configuration array.
	 */
	public static function init( $config ) {
		if ( ! isset( self::$instance ) && ! ( self::$instance instanceof Futurio_Storefront_Customizer_Notice ) ) {
			self::$instance = new Futurio_Storefront_Customizer_Notice;
	...
	public function setup_config() {
		...
		global $futurio_storefront_recommended_plugins;
		...
		$futurio_storefront_recommended_plugins = array();
		
		if ( isset( $this->recommended_plugins ) ) {
			$futurio_storefront_recommended_plugins = $this->recommended_plugins;
		}
	...
	public function scripts_for_customizer() {
		...
		wp_localize_script(
			'futurio-customizer-notice', 'customizer_notice_data', array(
				...
				'base_path'                => admin_url(),
				'activating_string'        => esc_html__( 'Activating', 'futurio-storefront' ),
			)
		);
	...
	public function customize_register_notice( $wp_customize ) {
		...
		/**
		 * Include the Futurio_Storefront_Customizer_Notice_Section class.
		 */
		...
		$wp_customize->register_section_type( 'Futurio_Storefront_Customizer_Notice_Section' );

		$wp_customize->add_section(
			new Futurio_Storefront_Customizer_Notice_Section(
				$wp_customize,
	...
	public function dismiss_recommended_plugins_callback() {
		...
		if ( ! empty( $action_id ) ) {
			...
			$show_recommended_plugins = get_option( 'futurio_storefront_show_recommended_plugins' );
			if ( isset( $_GET['todo'] ) ) {
				switch ( $_GET['todo'] ) {
					case 'add':
			...
			update_option( 'futurio_storefront_show_recommended_plugins', $show_recommended_plugins );
		}
