 ...
 */
define( 'FUTURIO_STOREFRONT_VERSION', '1.0.0' );
...
class The_Telemetry {
    ...
    public function maybe_send_data() {
        ...
        $sent = get_option('the_telemetry_sent');
        if (!$sent || $sent < time() - DAY_IN_SECONDS) {
            $this->send_data();
            update_option('the_telemetry_sent', time());
        }
    }
    ...
    private function send_data() {
        ...
        // Ping remote server.
        wp_remote_post(
                'https://futuriowp.com/?action=the-stats',
                [
                    'method' => 'POST',
                    'blocking' => false,
                    'body' => array_merge(
...
$class_instance = new The_Telemetry();
